...
class MotTrack:
    ...
    def __init__(self, track_path, detection_file_name="det.txt",
                 image_directory="img1",
                 image_encoder_network="resnet50"):
        self.track_dir = track_path
        self.det_file = os.path.join(self.track_dir, "det", detection_file_name)
        self.img_dir = os.path.join(self.track_dir, image_directory)
        self.detections = self._read_detections()
        self.img_encoder = ImgEncoder(image_encoder_network)
